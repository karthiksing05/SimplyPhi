for i in range(5):
    ...
    # Generating two sets of coefficients for two output variables
    coefficients1 = np.random.randn(3)
    coefficients2 = np.random.randn(3)

    # Generating two target variables (each with 3 bins after reshaping)
    y1 = X.dot(coefficients1) + np.random.randn(n_samples) * 0.5
    y2 = X.dot(coefficients2) + np.random.randn(n_samples) * 0.5

    # Ensure y1 and y2 are column vectors
    y1 = y1.reshape((-1, 1))
    y2 = y2.reshape((-1, 1))
    y = np.concatenate((y1, y2), axis=1)

    inputVars = [('cat', 2), ('num', 2), ('num', 2)]
    outputVars = [('num', 3), ('num', 3)]
    ...
    def phi_loss_func(model):
        ...
        def loss(y_true, y_pred):
            ...
            heatmapEvaluation += accuracy_dependency # make this add to everything
    ...
    with open(f"siaMultiMagTest{i}.pickle", "wb") as f:
