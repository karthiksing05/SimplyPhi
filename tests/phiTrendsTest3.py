...
def evaluate_tpm(tpm):
    labels = tuple([f"Node{i}" for i in range(TOTAL_NODES)])
    node_indices = tuple([i for i in range(TOTAL_NODES)])
    ...
    for test_state in all_test_states:
        ...
        print(f"EVALUATING FOR TEST STATE {str(test_state)}")
        print("-" * 20)

        major_complex_sia = -1
        EXPERIMENTAL_VERBOSE = True

        """
        MAIN OPTIMIZATION HERE! By inspection, there's basically no chance that the smaller subsystems will 
        have a greater Phi than the big subsystem, so we can effectively ignore them?

        This is really funny but I am growing impatient so really want to try!
        """

        subsystem = pyphi.Subsystem(network, test_state, node_indices)

        # for subsystem in pyphi.compute.network.subsystems(network, test_state):

        #     if len(subsystem.nodes) == 1:
        #         continue

        subsystem_node_indices = tuple([node.index for node in subsystem.nodes])

        subsystem_node_labels = pyphi.labels.NodeLabels(
            tuple([node.label for node in subsystem.nodes]),
            subsystem_node_indices
        )

        ces = pyphi.compute.ces(subsystem)

        best_subsystem_sia = -1
        for cut in pyphi.compute.subsystem.sia_bipartitions(subsystem_node_indices, node_labels=subsystem_node_labels):
            sia = pyphi.compute.subsystem.evaluate_cut(subsystem, cut, ces)
            if EXPERIMENTAL_VERBOSE and sia.phi > 0:
                print(subsystem)
                print(sia.cut)
                print(sia.phi)
                print()
            if (best_subsystem_sia == -1) or (sia.phi < best_subsystem_sia.phi):
                best_subsystem_sia = sia

        if (major_complex_sia == -1) or (best_subsystem_sia.phi > major_complex_sia.phi):
            major_complex_sia = best_subsystem_sia

        MCs.append(major_complex_sia)
        phi_avg += major_complex_sia.phi
...
with open("optim_nn_results.pickle", "wb") as f:
    pickle.dump([phi_avgs, all_MCs, model, X, y], f)
